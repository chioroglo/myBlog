...
const ProfilePage = () => {
    ...
    const [user,setUser] = useState<UserModel>({id: 0,registrationDate: new Date(),lastActivity: new Date(),username:"",fullName:""});

    const [visibleTabIndex,setVisibleTabIndex] = useState<number>(0);    

    let [pageRequest,setPageRequest] = useState<CursorPagedRequest>();

    const fetchUser = () =>  userApi.getUserById(parseInt(userId || "0")).then(response => { setUser(response.data); return response.data});

    const handleTabChange = (event: React.SyntheticEvent,newValue: number) => { event.preventDefault(); setVisibleTabIndex(newValue)};

    const tabProps = (index:number) => { return {id: `simple-tab-${index}` ,'aria-controls': `simple-tabpanel-${index}`} };
            ...
            <>
                ...
                <Box style={{width:"70vw",margin:"0 auto"}}>
                    ...
                    <CustomTabPanel index={0} value={visibleTabIndex}>
                        {pageRequest && <BlogReel reelWidth="100%" pageSize={DefaultPageSize} pagingRequestDefault={pageRequest} availableFilterNames={[]}/>}
                    </CustomTabPanel>
